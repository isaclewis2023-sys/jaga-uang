...
// SVG-based human-like face with smooth Framer Motion animations
export default function AriaFace({ expression, isTalking }: AriaFaceProps) {
  ...
  const accentColor = isWarning ? '#ff2055' : isSad ? '#00e5ff' : '#00ff41'
  const glowColor = isWarning ? 'rgba(255,32,85,0.4)' : isSad ? 'rgba(0,229,255,0.3)' : 'rgba(0,255,65,0.35)'

  return (
    <div className="relative flex items-center justify-center select-none" style={{ width: 180, height: 200 }}>
      {/* Outer glow ring */}
      <motion.div
        className="absolute inset-0 rounded-full"
        animate={{
          boxShadow: isTalking
            ? [`0 0 30px ${glowColor}`, `0 0 55px ${glowColor}`, `0 0 30px ${glowColor}`]
            : [`0 0 20px ${glowColor}`, `0 0 30px ${glowColor}`, `0 0 20px ${glowColor}`],
        }}
        transition={{ duration: isTalking ? 0.35 : 2.5, repeat: Infinity, ease: 'easeInOut' }}
      ...
      >
        <defs>
          <radialGradient id="faceGrad" cx="50%" cy="45%" r="55%">
            <stop offset="0%" stopColor={isWarning ? '#1a0005' : isSad ? '#001a1a' : '#001a00'} />
        ...
        {/* Face base */}
        <ellipse cx="80" cy="88" rx="68" ry="78" fill="url(#faceGrad)" stroke={accentColor} strokeWidth="1" strokeOpacity="0.4" />
      ...
      >
        ...
        >
          {expression === 'thinking' ? 'MEMPROSES...' : expression === 'talking' ? 'ARIA ONLINE' : expression === 'happy' ? 'ARIA ONLINE' : expression === 'warning' ? 'PERHATIAN' : expression === 'sad' ? 'ARIA ONLINE' : 'ARIA ONLINE'}
        </span>
      </motion.div>
    </div>
  )
