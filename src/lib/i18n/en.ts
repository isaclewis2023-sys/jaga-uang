const en = {
  common: {
    appName: 'Jaga Uang',
    tagline: 'Financial Terminal',
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    search: 'Search',
    filter: 'Filter',
    all: 'All',
    close: 'Close',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    back: 'Back',
    next: 'Next',
    submit: 'Submit',
    required: 'Required',
    optional: 'Optional',
    success: 'Success',
    error: 'Error',
    noData: 'No data',
  },
  auth: {
    accessTerminal: 'ACCESS TERMINAL',
    enterCode: 'ENTER ACCESS CODE',
    password: 'Password',
    login: 'Login',
    logout: 'Logout',
    wrongPassword: 'Wrong password. Try again.',
    setupTitle: 'SYSTEM INITIALIZATION',
    setupDesc: 'Create a password to secure your financial terminal.',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    passwordMismatch: 'Passwords do not match',
    passwordTooShort: 'Password must be at least 8 characters',
    setupComplete: 'SYSTEM INITIALIZED',
    scanning: 'SCANNING...',
    verifying: 'VERIFYING...',
    accessGranted: 'ACCESS GRANTED',
    accessDenied: 'ACCESS DENIED',
  },
  nav: {
    dashboard: 'Dashboard',
    transactions: 'Transactions',
    accounts: 'Accounts',
    reports: 'Reports',
    budget: 'Budget',
    goals: 'Goals',
    settings: 'Settings',
  },
  dashboard: {
    title: 'Dashboard',
    netWorth: 'Net Worth',
    totalIncome: 'Total Income',
    totalExpense: 'Total Expense',
    netBalance: 'Net Balance',
    savingsRate: 'Savings Rate',
    thisMonth: 'This Month',
    recentTransactions: 'Recent Transactions',
    viewAll: 'View All',
    healthScore: 'Financial Health Score',
    budgetOverview: 'Budget Overview',
    accountsOverview: 'Accounts Overview',
    incomeVsExpense: 'Income vs Expense',
    last30Days: 'Last 30 Days',
    netWorthTrend: 'Net Worth Trend',
    trendUp: 'Up',
    trendDown: 'Down',
    trendFlat: 'Flat',
  },
  transactions: {
    title: 'Transactions',
    addTransaction: 'Add Transaction',
    editTransaction: 'Edit Transaction',
    deleteTransaction: 'Delete Transaction',
    income: 'Income',
    expense: 'Expense',
    amount: 'Amount',
    description: 'Description',
    category: 'Category',
    account: 'Account',
    date: 'Date',
    notes: 'Notes',
    type: 'Type',
    deleteConfirm: 'Are you sure you want to delete this transaction?',
    noTransactions: 'No transactions yet',
    addFirst: 'Add your first transaction',
    requiredFields: 'All fields are required',
    saveFailed: 'Failed to save',
    today: 'Today',
    last7Days: '7 Days',
    thisMonth: 'This Month',
    lastMonth: 'Last Month',
    customRange: 'Custom',
    export: 'Export',
    exportCSV: 'Export CSV',
    exportExcel: 'Export Excel',
    duplicateWarning: 'Similar transaction found',
    added: 'Transaction added',
    filterSummary: 'Income: {income} · Expense: {expense} · Net: {net}',
  },
  accounts: {
    title: 'Accounts',
    addAccount: 'Add Account',
    editAccount: 'Edit Account',
    deleteAccount: 'Delete Account',
    bank: 'Bank',
    cash: 'Cash',
    savings: 'Savings',
    investment: 'Investment',
    credit: 'Credit',
    balance: 'Balance',
    type: 'Type',
    name: 'Account Name',
    totalAssets: 'Total Assets',
    totalLiabilities: 'Total Liabilities',
    transfer: 'Transfer',
    transferBetweenAccounts: 'Transfer Between Accounts',
    from: 'From',
    to: 'To',
    noAccounts: 'No accounts yet',
    addFirst: 'Add your first account',
    deactivateConfirm: 'Are you sure you want to deactivate this account?',
    sameAccountError: 'Source and destination account cannot be the same',
  },
  reports: {
    title: 'Reports',
    period: 'Period',
    oneMonth: '1 Month',
    threeMonths: '3 Months',
    sixMonths: '6 Months',
    oneYear: '1 Year',
    custom: 'Custom',
    startDate: 'Start Date',
    endDate: 'End Date',
    totalIncome: 'Total Income',
    totalExpense: 'Total Expense',
    netAmount: 'Net',
    avgPerMonth: 'Avg/Month',
    categoryBreakdown: 'Category Breakdown',
    monthlyComparison: 'Monthly Comparison',
    cashFlow: 'Cash Flow',
    spendingHeatmap: 'Spending Heatmap',
    netWorthHistory: 'Net Worth History',
    netWorthNote: 'Calculated from historical transaction reconstruction',
    overview: 'Overview',
    netWorthTab: 'Net Worth',
  },
  budget: {
    title: 'Budget',
    addBudget: 'Add Budget',
    editBudget: 'Edit Budget',
    deleteBudget: 'Delete Budget',
    category: 'Category',
    limit: 'Limit',
    spent: 'Spent',
    remaining: 'Remaining',
    month: 'Month',
    year: 'Year',
    onTrack: 'On Track',
    warning: 'Warning',
    exceeded: 'Exceeded',
    noBudgets: 'No budgets yet',
    addFirst: 'Start setting your budget',
  },
  goals: {
    title: 'Financial Goals',
    addGoal: 'Add Goal',
    editGoal: 'Edit Goal',
    deleteGoal: 'Delete Goal',
    addFunds: 'Add Funds',
    deleteConfirm: 'Are you sure you want to delete this goal?',
    fromAccount: 'From Account',
    insufficientBalance: 'Insufficient account balance',
    fundAdded: 'Funds added successfully',
    name: 'Goal Name',
    target: 'Target',
    current: 'Collected',
    remaining: 'Remaining',
    deadline: 'Deadline',
    daysLeft: 'Days Left',
    monthlyNeeded: 'Needed/Month',
    inProgress: 'In Progress',
    achieved: 'Achieved',
    overdue: 'Overdue',
    noGoals: 'No goals yet',
    addFirst: 'Create your first financial goal',
    progress: 'Progress',
    description: 'Description',
  },
  settings: {
    title: 'Settings',
    categories: 'Categories',
    security: 'Security',
    language: 'Language',
    data: 'Data',
    changePassword: 'Change Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    exportData: 'Export Data',
    exportDesc: 'Download all data in JSON format',
    download: 'Download',
    recurring: 'Recurring Transactions',
    addCategory: 'Add Category',
    addRecurring: 'Add Recurring Transaction',
    frequency: 'Frequency',
    daily: 'Daily',
    weekly: 'Weekly',
    monthly: 'Monthly',
    yearly: 'Yearly',
    startDate: 'Start Date',
    endDate: 'End Date',
    nextDue: 'Next Due',
    active: 'Active',
    inactive: 'Inactive',
    noRecurring: 'No recurring transactions yet',
  },
  health: {
    score: 'Score',
    savingsRate: 'Savings Rate',
    budgetAdherence: 'Budget Adherence',
    emergencyFund: 'Emergency Fund',
    goalProgress: 'Goal Progress',
    excellent: 'EXCELLENT',
    good: 'GOOD',
    warning: 'WARNING',
    critical: 'CRITICAL',
    months: 'Months',
  },
} as const

export default en
